...
abstract class WechatCommonRequest extends WeChatRequest
{
    private $tag_msgid = 'MsgId';

    public $msg_id;

    abstract function _handle_node_sub($param);
    ...
    function _handle_node($param)
    {
        ...
        switch ($name) {
            case $this->tag_msgid:
                ...
                break;
            default:
                $this->_handle_node_sub($param);
                break;
        }
    }
}

class WeChatTextRequest extends WechatCommonRequest
{
    private $tag_content = 'Content';

    public $content;

    function _handle_node_sub($param)
    {
        $value = $param->nodeValue;
        $name = $param->nodeName;
        switch ($name) {
...
class WechatImageRequest extends WechatCommonRequest{
...
class WechatVoiceRequest extends WechatCommonRequest
{
...
class WechatVideoRequest extends WechatCommonRequest
{
...
class WechatLocationRequest extends WechatCommonRequest
{
...
class WeChatLinkRequest extends WechatCommonRequest
{
...
class WeChatRequestBuilder
{
    ...
    private $request_image = 'image';
    private $request_voice = 'voice';
    private $request_video = 'video';
    private $request_location = 'location';
    ...
    function builder($xml)
    {
        ...
        switch ($msg_type) {
            case $this->request_text:
                $object = new WeChatTextRequest();
                break;
            case $this->request_event:
                $object = new WeChatEventRequest();
                break;
            case $this->request_image:
                $object = new WechatImageRequest();
                break;
        }
...
class WeChatArticleResponse extends WebChatResponse
{
    ...
    function add_article($title='', $description='', $pic_url='', $url='')
    {
        ...
        $this->articles = array_merge($this->articles, $tmp);
    }
}
