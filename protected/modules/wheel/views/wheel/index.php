                    ...
                    <div class="tabbable">
                        <ul id="myTab" class="nav nav-tabs">
                            <li class="<?php if ($type == GiftModel::TYPE_KEYWORDS) { ?>active<?php } ?>">
                                <a href="<?php echo Yii::app()->createUrl('wheel', array('type' => GiftModel::TYPE_KEYWORDS)) ?>">
                                    关键词大转盘列表
                                </a>
                            </li>
                            <?php if ($wechatInfo->isAuth) { ?>
                                <li class="<?php if ($type == GiftModel::TYPE_MENU) { ?>active<?php } ?>">
                                    <a href="<?php echo Yii::app()->createUrl('wheel', array('type' => GiftModel::TYPE_MENU)) ?>">
                                        菜单大转盘列表
                                    </a>
                                </li>
                            <?php } ?>
                            <li>
                                <a href="<?php echo Yii::app()->createUrl('/wheel/wheel/create', array('type' => $type)) ?>"
                                    ...
                                    <table class="table table-striped table-bordered table-hover" id="sample-table-1">
                                        ...
                                        foreach ($data as $d) {
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <?php if ($type == GiftModel::TYPE_KEYWORDS) { ?>
                                                        <?php foreach ($d->wheel_keywords as $keywords) { ?>
                                                            <span
                                                                class="label label-sm label-primary arrowed arrowed-right"><?= $keywords->name ?></span>
                                                        <?php } ?>
                                                    <?php } else { ?>
                                                        <?php foreach ($d->wheel_menuaction as $menuaction) { ?>
                                                            <span
                                                ...
                                                <?php if ($type == GiftModel::TYPE_KEYWORDS) { ?>
                                                    <td>
                                                        <?php foreach ($d->wheel_keywords as $isAccurate) {
                                                ...
                                                <td style="width:23%">
                                                    <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                                        <?php if ($d->isSensitive) { ?>
                                                            <a class="btn btn-xs btn-primary"
                                                               href="<?php echo Yii::app()->createUrl('wheel/wheel/codes/id/' . $d->id) ?>">
                                                                <i class="fa fa-list-ol bigger-120">礼品码</i>
                                                            </a>
                                                        <?php } ?>
                                                        <?php if (!$d->status) { ?>
                                                        ...
                                                        <a class="btn btn-xs btn-info" id="winner"
                                                           rel="<?php echo Yii::app()->createUrl('wheel/wheel/winnerList/id/'.$d->id) ?>">中奖查询</a>
                                                        <!--<a class="btn btn-xs btn-danger  bootbox-confirm" rel="<?= Yii::app()->createUrl('wheel/wheel/delete/id/' . $d->id) ?>">
