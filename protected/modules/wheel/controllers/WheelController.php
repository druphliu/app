 ...
 * Time: 11:04
 */
class WheelController extends WechatManagerController
{
    public function actionIndex()
    {
        ...
        $dataProvider = new CActiveDataProvider('WheelModel', array(
            'criteria' => array(
                'order' => 't.id DESC',
                'with' => $with,
    ...
    public function actionCreate()
    {
        ...
        if (isset($_POST['WheelModel'])) {
            ...
            if ($_FILES) {
                $file = array('name' => $_FILES['WheelModel']['name']['backgroundPic'],
                    'type' => $_FILES['WheelModel']['type']['backgroundPic'],
                    'tmp_name' => $_FILES['WheelModel']['tmp_name']['backgroundPic'],
                    'error' => $_FILES['WheelModel']['error']['backgroundPic'],
                    'size' => $_FILES['WheelModel']['size']['backgroundPic']);
                ...
                $file = array('name' => $_FILES['WheelModel']['name']['button'],
                    'type' => $_FILES['WheelModel']['type']['button'],
                    'tmp_name' => $_FILES['WheelModel']['tmp_name']['button'],
                    'error' => $_FILES['WheelModel']['error']['button'],
                    'size' => $_FILES['WheelModel']['size']['button']);
                $uploadPath = Yii::app()->params['scratchPath'] . "/" . $this->wechatInfo->id . '/';
                $fileUpload = new FileUpload($uploadPath, $file);
                $fileUpload->move();
                $result = $fileUpload->getMessages();
