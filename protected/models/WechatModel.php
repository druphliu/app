...
class WechatModel extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
            array('originalId','unique'),
			array('uid, originalId, name, wechatAccount, token, apiUrl', 'required'),
			array('uid, isAuth, area, created_at, updated_at', 'numerical', 'integerOnly'=>true),
			...
			array('appid, secret', 'length', 'max'=>150),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, uid, type, isAuth, name, originalId, wechatAccount, area, created_at, updated_at, token, apiUrl, appid, secret', 'safe', 'on'=>'search'),
		);
	}
